...
";
	...
	public string SimpleCRUD_MySql()
	{
		...
		using( DbContext mysql = new DbContext("mysql") ) {
			...
			Product test = DbHelper.GetDataItem<Product>("select * from user limit 0, 1", null, mysql, CommandKind.SqlTextNoParams);
			if( test == null ) {
				using( DbContext access = new DbContext("access") ) {
					List<Product> products = DbHelper.FillList<Product>("select * from user ", null, access, CommandKind.SqlTextNoParams);
